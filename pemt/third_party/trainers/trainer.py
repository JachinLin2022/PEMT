...
class BaseTrainer(Trainer):
    def __init__(self, evaluation_metrics=[], multi_task_compute_metrics=None, data_info=None, *args, **kwargs):
    ...
    def evaluate(
        ...
        if len(self.evaluation_metrics) != 0:
            selected_metrics = [output.metrics[metric_key_prefix+"_"+k]
                                for k in self.evaluation_metrics if metric_key_prefix+"_"+k in output.metrics]
            assert len(
                selected_metrics) >= 1, "at least one metric should be selected to compute the average_metrics."
            output.metrics.update(
                {metric_key_prefix+'_average_metrics': np.mean(selected_metrics)})
